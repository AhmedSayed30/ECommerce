...
class FirebaseAuthRepositoryImpl(private val auth: FirebaseAuth = FirebaseAuth.getInstance(),
    ...
    override suspend fun loginWithGoogle(idToken: String): Flow<Resource<UserDetailsModel>> = login(AuthProvider.GOOGLE) {
    ...
    private suspend fun login(
        ...
        signInResult: suspend () -> AuthResult
    ):
            Flow<Resource<UserDetailsModel>> = flow {
        try {
            emit(Resource.Loading())
            ...
            val authResult = signInResult()
            val userId = authResult.user?.uid

            if (userId == null) {
                val msg = "sign in User Id not found"
    ...
    private fun logAuthIssueToCrashlytics(msg: String, provider: String) {
        CrashlyticsUtils.sendCustomLogToCrashlytics<LoginException>(msg,
            CrashlyticsUtils.LOGIN_KEY to msg,
            CrashlyticsUtils.LOGIN_KEY to provider)
